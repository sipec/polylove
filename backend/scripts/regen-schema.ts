...
async function getTableInfo(pg: SupabaseDirectClient, tableName: string) {
  ...
  const primaryKeys = await pg.manyOrNone<{
    name: string
    column_name: string
  }>(
    `SELECT 
...
async function generateSQLFiles(pg: SupabaseDirectClient) {
  ...
  for (const tableInfo of tableInfos) {
    ...
    if (pkeys.length > 1) {
      const name = pkeys[0].name
      const cols = pkeys.map((p) => p.column_name)
      content += `  CONSTRAINT ${name} PRIMARY KEY (${cols.join(', ')}),\n`
    }
    ...
    if (tableInfo.foreignKeys.length > 0)
      content += `-- Foreign Keys (in foreign_keys.sql)\n`
    for (const fk of tableInfo.foreignKeys) {
      content += `-- ALTER TABLE ${tableInfo.tableName}\n`
      content += `-- ADD CONSTRAINT ${fk.constraint_name} ${fk.definition};\n`
      content += '--\n'
    }
    ...
    for (const index of tableInfo.indexes) {
      const commentOut = pkeys.some((pkey) => pkey.name === index.index_name)
      if (commentOut) content += `/*\n`
  ...
  console.log('Writing foreign keys to foreign_keys.sql')
  let fkeysContent = `-- This file is autogenerated from regen-schema.ts\n\n`
  for (const t of tableInfos)
    for (const fk of t.foreignKeys)
      fkeysContent += `ALTER TABLE ${t.tableName} ADD CONSTRAINT ${fk.constraint_name} ${fk.definition};\n`

  await fs.writeFile(`${outputDir}/foreign_keys.sql`, fkeysContent)
